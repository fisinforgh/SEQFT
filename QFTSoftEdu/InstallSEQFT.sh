...
#This script install QFT_Software
...
if [ $cont -eq 1 ]; then
	...
	echo "$pathROOTCONFIG"
	echo " ")
    echo "$infoINS1" | (zenity --text-info --title="SEQFT install" --width=800 --height=400 --ok-label='CONTINUE' --cancel-label='Exit install')
...
if [ -f "$FILE" ]; then
    ...
    (zenity --info --text="SEQFT_1.0 executable compilation complete $FILE. \nPlease click CONTINUE to setup QFT executable globally and desktop icon "\
	    --title="SEQFT install" --ok-label='CONTINUE' --width=800 --height=400 )  
else 
...
echo "Version=2.0" >> "$deskFILE"
echo "Type=Application" >> "$deskFILE"
echo "Exec=$dirPATH/$nameDIR/$FILE" >> "$deskFILE"
echo "Icon=$dirPATH/$nameDIR/Icons/logoQFT.png" >> "$deskFILE"
