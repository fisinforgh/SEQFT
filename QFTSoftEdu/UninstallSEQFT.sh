...
messageUNIS=$(echo "Welcome to the QFTSoftware UNINSTALL wizard"
	      echo "You are about to uninstall  QFTSoftware software"
	      echo "If you are completely sure, please clik on UNINSTALL"
	      )

echo "$messageUNIS" | (zenity --text-info --title="QFTSoftware uninstall" --width=800 --height=400 --ok-label='UNINSTALL' --cancel-label='Exit')
if [ $? -eq 1 ]; then
    echo "Exit"
    exit 2
fi
...
(zenity --info --text="Please click CONTINUE to select \"Desktop\" Folder to UNISTALL QFTSoftware desktop's icon" --title="QFTSoftware UNINSTALL desktop icon" --ok-label='CONTINUE' --width=800 --height=400 )

dirDESK=$(zenity --file-selection   --directory --title="QFTSoftware UNINSTALL choose a Desktop Folder" --width=800 --height=400)

manUNIS=$( echo "Remove QFTSoftware manually:"
...
hiddenFILE=".pathDIR_QFTSOFT.txt"
dirNAME="QFTSOFT"
tarFILE="QFTSOFT.tar"
